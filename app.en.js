...
function byId(id) {
  return document.getItemById(id);
}
...
function collectParams() {
  ...
  byId("termYearsInfo").value = termMonths % 12 === 0 ? `${termMonths / 12} years` : "";
...
function calculateAll() {
  ...
  status.textContent = invalid.length > 0
    ? `Done, but skipped invalid schedule entries: ${invalid.join(" | ")}`
    : `Done: ${new Date().toLocaleTimeString("en-US")}`;
}
...
function renderSummary(model, summaryMonth) {
  ...
  summary.innerHTML = "";
  summary.appendChild(kpi("Base installment (base WIBOR)", `${fmtMoney.format(model.mortgages.base.monthlyPayment)} PLN`, "PMT: margin + base WIBOR, annuity payment until payoff month."));
  summary.appendChild(kpi(`Rent wealth (m-c ${summaryMonth})`, `${fmtMoney.format(rentAtMonth)} PLN`, "After-tax investment portfolio with starting capital and monthly contributions."));
  summary.appendChild(kpi(`Buy wealth (m-c ${summaryMonth})`, `${fmtMoney.format(buyBaseAtMonth)} PLN`, "Net sale proceeds + buy-side investments - remaining mortgage balance - entry cost."));
  summary.appendChild(kpi(`Base difference (m-c ${summaryMonth})`, `${fmtMoney.format(diffBaseAtMonth)} PLN`, "Base buy result minus rent result."));
  summary.appendChild(kpi("Crossing (base)", crossBase ? `${crossBase} m-c` : "none in horizon", "First month when buying outperforms renting."));
  summary.appendChild(kpi("Full mortgage payoff", `${payoffMonth} m-c`, "After this month, schedule, chart horizon and further investing are frozen in this comparison."));
  summary.appendChild(kpi("Buy portfolio at payoff", `${fmtMoney.format(buyPortfolioAtPayoff)} PLN`, "Investment value on the buy side at full payoff."));
  summary.appendChild(kpi("Rent portfolio at the same date", `${fmtMoney.format(rentPortfolioAtPayoff)} PLN`, "Renter portfolio value at base mortgage payoff date."));
  summary.appendChild(kpi(`Property value (m-c ${summaryMonth})`, `${fmtMoney.format(baseBuyRow ? baseBuyRow.homeValue : 0)} PLN`, "Property value after annual growth."));
  summary.appendChild(kpi(`Total mortgage outflow (m-c ${summaryMonth})`, `${fmtMoney.format(mortgageBaseRow ? mortgageBaseRow.cumulativeCashOut : 0)} PLN`, "Total installments and overpayments up to selected month."));
  summary.appendChild(kpi(`Owner costs (m-c ${summaryMonth})`, `${fmtMoney.format(mortgageBaseRow ? mortgageBaseRow.cumulativeOwnerCosts : 0)} PLN`, "Insurance, minor repairs, owner maintenance."));
  summary.appendChild(kpi(`Total rent outflow (m-c ${summaryMonth})`, `${fmtMoney.format(rentRow ? rentRow.cumulativeRentCost : 0)} PLN`, "Rent plus additional rent costs up to selected month."));
  summary.appendChild(kpi(`Total overpaid (m-c ${summaryMonth})`, `${fmtMoney.format(mortgageBaseRow ? mortgageBaseRow.cumulativeOverpay : 0)} PLN`, "Monthly surplus and schedule-based overpayments used by mortgage."));
}
...
function renderDiagnostics(model, summaryMonth) {
  ...
  const rows = [
    ["Value mieszkania brutto", buyRow.homeValue, "Value nieruchomosci po rocznej zmianie ceny."],
    ["Mortgage balance", mortgageRow.endBalance, "Remaining principal balance in base scenario."],
    ["Equity before sale", buyRow.equityBeforeSale, "Value mieszkania minus pozostale saldo kredytu."],
    ["Sale cost", buyRow.saleCostAmount, "Percent of current property value."],
    ["Entry cost", buyRow.entryCostAmount, "One-time purchase cost calculated from starting property value."],
    ["Buyer portfolio", buyRow.investmentAfterTax, "Buy-side invested funds: unused overpayments and positive advantage of rent cost over mortgage cost."],
    ["Renter portfolio", rentRow.investmentAfterTax, "Starting capital = down payment, plus monthly surplus and positive difference of mortgage housing cost over rent."],
    ["Renter investment contributions", rentRow.cumulativeContribution, "Suma miesiecznych doplat do portfela najemcy, bez kapitalu startowego."],
    ["Buyer investment contributions", buyRow.cumulativeContribution, "Suma miesiecznych doplat do portfela kupujacego."],
    ["Mortgage cash outflow", mortgageRow.cumulativeCashOut, "Total installments and overpayments up to selected month."],
    ["Owner costs", mortgageRow.cumulativeOwnerCosts, "Owner maintenance costs added monthly."],
    ["Rent cash outflow", rentRow.cumulativeRentCost, "Total rent and additional rent costs up to selected month."],
    ["Buy net wealth", buyRow.wealth, "Net property sale value + buyer portfolio - mortgage balance - entry cost."],
    ["Rent net wealth", rentRow.wealth, "Renter portfolio po podatku, przy tych samych zalozeniach inwestycyjnych."],
    ["Difference buy - rent", buyRow.wealth - rentRow.wealth, "Positive value means buying is better in selected month."],
  ];

  for (const [label, value, note] of rows) {
    const tr = document.createItem("tr");
...
function kpi(title, value, note) {
  const box = document.createItem("div");
  box.className = "kpi";
...
function renderSensitivityTable(params, summaryMonth, scheduleMap) {
  ...
  const headerRow = document.createItem("tr");
  headerRow.innerHTML = `<th>Home growth / base WIBOR</th>${wiborDeltas.map((d) => `<th>${(params.wiborBasePct + d).toFixed(1)}%</th>`).join("")}`;
...
function renderChart(containerId, title, buySeries, rentSeries, payoffMonth, viewMonths) {
  ...
  const svg = document.createItemNS("http://www.w3.org/2000/svg", "svg");
  svg.setAttribute("viewBox", `0 0 ${W} ${H}`);
  const add = (tag, attrs = {}, parent = svg) => {
    const el = document.createItemNS("http://www.w3.org/2000/svg", tag);
  ...
  const legend = document.createItem("div");
  legend.className = "chart-legend-out";
  const crossText = crossMonth === null
    ? "Crossing: none"
    : `Crossing: ${crossMonth} m (Buy > Rent)`;
  legend.innerHTML = `
    <div class="legend-row"><span class="legend-line buy"></span><span>Buy (${title})</span></div>
    <div class="legend-row"><span class="legend-line rent"></span><span>Rent</span></div>
    <div class="legend-row"><span class="legend-line cross"></span><span>${crossText}</span></div>
    <div class="legend-note">X axis: sale month (numbers below chart).</div>
    <div class="legend-note">Y axis: net wealth [PLN] (numbers on the left).</div>
  `;
